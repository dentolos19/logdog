...
def _ensure_log_group_profile_column() -> None:
    inspector = inspect(engine)
    table_names = inspector.get_table_names()
    if "log_groups" not in table_names:
        return

    column_names = {column["name"] for column in inspector.get_columns("log_groups")}
    if "profile_name" in column_names:
        return

    with engine.begin() as connection:
        connection.execute(text("ALTER TABLE log_groups ADD COLUMN profile_name VARCHAR(255) NULL"))
        connection.execute(text("UPDATE log_groups SET profile_name = 'default' WHERE profile_name IS NULL"))
