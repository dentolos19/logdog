...
import pytest
from lib.ai import LlmSchemaResponse
...
class TestDdlGeneration:
    def test_generated_ddl_is_postgres_compatible(self, service: LogPreprocessorService) -> None:
        """Verify generated DDL follows PostgreSQL-compatible CREATE TABLE conventions."""

        file_input = FileInput(filename="test.log", content="\n".join(SYSLOG_SAMPLE))
        result = service.preprocess([file_input])

        assert result.ddl.startswith("CREATE TABLE IF NOT EXISTS")
        assert f'"{result.table_name}"' in result.ddl
        assert '"id" BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY' in result.ddl
    ...
    def test_all_generated_table_ddls_are_valid(self, service: LogPreprocessorService) -> None:
        ...
        ddl_by_table = {table.table_name: table.ddl for table in result.generated_tables}

        assert service.table_name in ddl_by_table
        assert f"{service.table_name}_file-a" in ddl_by_table
        assert f"{service.table_name}_file-b" in ddl_by_table

        for table_name, ddl in ddl_by_table.items():
            assert ddl.startswith("CREATE TABLE IF NOT EXISTS")
            assert f'"{table_name}"' in ddl
