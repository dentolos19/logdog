...
SUPPORTED_EXTENSIONS = {
    '.csv', '.tsv', '.json', '.parquet', '.xml', '.yaml', '.yml',
    '.log', '.txt', '.out', '.bin', '.dat', '.blob', '.wal', '.blg',
    '.pcap', '.pcapng', '.cap', '.pb', '.thrift', '.avro', '.msgpack',
    '.evtx', '.evt', '.dmp', '.core', '.mdmp'
}

ARCHIVE_EXTENSIONS = {
    '.gz', '.bz2', '.xz', '.lzma', '.zip', '.tar', '.7z', '.rar'
}


def validate_file(file_path: str) -> Tuple[bool, str]:
    p = Path(file_path)
    if not p.exists(): return False, f"Not found: {file_path}"
    if not p.is_file(): return False, f"Not a file: {file_path}"
    if p.suffix.lower() not in SUPPORTED_EXTENSIONS: return False, f"Unsupported: {p.suffix}"
    if not os.access(p, os.R_OK): return False, "Not readable"
    if p.stat().st_size == 0: return False, "Empty"
    return True, str(p)
...
def detect_archive_type(file_path: str) -> Optional[str]:
    ...
    return {'.zip': 'zip', '.tar': 'tar', '.gz': 'gzip', '.bz2': 'bzip2', 
            '.xz': 'xz', '.lzma': 'xz', '.7z': '7z', '.rar': 'rar'}.get(ext)
...
def extract_archive(archive_path: str, output_dir: Optional[str] = None) -> Tuple[bool, str, List[str]]:
    ...
    try:
        if arch_type == 'zip':
            ...
            with f_cls(archive_path, 'rb') as f: out.write_bytes(f.read())
            return True, output_dir, [str(out)]
        elif arch_type == '7z':
            if not py7zr: return False, "py7zr not installed", []
            with py7zr.SevenZipFile(archive_path) as z:
                z.extractall(output_dir)
                return True, output_dir, z.getnames()
        elif arch_type == 'rar':
            if not rarfile: return False, "rarfile not installed", []
...
def _find_dotnet_parser() -> Optional[Path]:
    ...
    if _DOTNET_PARSER is False: return None
    if _DOTNET_PARSER: return _DOTNET_PARSER
    for p in [
...
class AIClient:
    BASELINE_COLS = [
        InferredColumn(name="id", sql_type=SqlType.INTEGER, description="Auto-incrementing primary key for each parsed record.", nullable=False, kind=ColumnKind.BASELINE),
        InferredColumn(name="timestamp", sql_type=SqlType.TEXT, description="Normalized ISO-8601 timestamp of the log event, if detectable.", kind=ColumnKind.BASELINE),
        InferredColumn(name="timestamp_raw", sql_type=SqlType.TEXT, description="Original timestamp string exactly as it appeared in the log.", kind=ColumnKind.BASELINE),
        InferredColumn(name="source", sql_type=SqlType.TEXT, description="Identifier for the source of the log (e.g., filename, hostname, service name).", kind=ColumnKind.BASELINE),
        InferredColumn(name="source_type", sql_type=SqlType.TEXT, description="Category of the source (e.g., 'file', 'stream', 'api').", kind=ColumnKind.BASELINE),
        InferredColumn(name="log_level", sql_type=SqlType.TEXT, description="Severity level of the log entry (e.g., INFO, WARN, ERROR).", kind=ColumnKind.BASELINE),
        InferredColumn(name="event_type", sql_type=SqlType.TEXT, description="Classified type of the event (e.g., 'request', 'error', 'metric').", kind=ColumnKind.BASELINE),
        InferredColumn(name="message", sql_type=SqlType.TEXT, description="Primary human-readable message content of the log entry.", kind=ColumnKind.BASELINE),
        InferredColumn(name="raw_text", sql_type=SqlType.TEXT, description="Complete original text of the log record, preserved for traceability.", nullable=False, kind=ColumnKind.BASELINE),
        InferredColumn(name="record_group_id", sql_type=SqlType.TEXT, description="Identifier linking related records from the same multiline cluster or transaction.", kind=ColumnKind.BASELINE),
        InferredColumn(name="line_start", sql_type=SqlType.INTEGER, description="1-based line number where this record starts in the source file.", kind=ColumnKind.BASELINE),
        InferredColumn(name="line_end", sql_type=SqlType.INTEGER, description="1-based line number where this record ends in the source file.", kind=ColumnKind.BASELINE),
        InferredColumn(name="parse_confidence", sql_type=SqlType.REAL, description="Confidence score (0.0-1.0) of how accurately this record was parsed.", kind=ColumnKind.BASELINE),
        InferredColumn(name="schema_version", sql_type=SqlType.TEXT, description="Version of the schema used to parse this record.", kind=ColumnKind.BASELINE),
        InferredColumn(name="additional_data", sql_type=SqlType.TEXT, description="JSON object containing any extra fields that did not map to named columns.", kind=ColumnKind.BASELINE),
    ]
    ...
    def process_chunk(self, content: str, metadata: dict, line_num: int = 1) -> Optional[dict]:
        ...
        try:
            ...
            extra_data = {k: v for k, v in ai_data.items() if k not in ['timestamp', 'log_level', 'event_type', 'message', 'parse_confidence']}
            
            return {
                col.name: (
                    ai_data.get('timestamp') if col.name == 'timestamp' else
                    content[:50] if col.name == 'timestamp_raw' else
                    metadata.get('source', 'unknown') if col.name == 'source' else
                    metadata.get('type', 'unknown') if col.name == 'source_type' else
                    ai_data.get('log_level', 'INFO') if col.name == 'log_level' else
                    ai_data.get('event_type', 'log') if col.name == 'event_type' else
                    ai_data.get('message', content[:200]) if col.name == 'message' else
                    content if col.name == 'raw_text' else
                    line_num if col.name == 'line_start' else
                    line_num if col.name == 'line_end' else
                    ai_data.get('parse_confidence', 0.5) if col.name == 'parse_confidence' else
                    '1.0' if col.name == 'schema_version' else
                    json.dumps(extra_data) if col.name == 'additional_data' else
                    None
                )
