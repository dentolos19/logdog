  ...
  return (
    <ScrollArea className={"flex-1"}>
      <div className={"flex flex-col gap-4 p-6"}>
        ...
        <input className={"hidden"} multiple={true} onChange={onFileChange} ref={fileInputRef} type={"file"} />
        ...
        {selectedFiles.length > 0 && (
          <div className={"flex items-center justify-between gap-3"}>
            ...
            <Button disabled={isParsing} onClick={onRun} size={"sm"}>
      ...
      {semFields.length > 0 && (
        <div className={"flex flex-wrap gap-2"}>
          {semFields.map(({ label, value }) => (
            <div className={"flex items-center gap-1 rounded-sm bg-muted px-2 py-1"} key={label}>
              <span className={"text-[10px] text-muted-foreground"}>{label}:</span>
              <span className={"font-medium font-mono text-[10px]"}>{value}</span>
            </div>
          ))}
        </div>
      )}
